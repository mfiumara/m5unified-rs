...
use m5unified::{M5Unified, StackChanServos, YAW_ZERO_DEFAULT, PITCH_ZERO_DEFAULT};
...
fn draw_ui(
    ...
    let _ = d.print("  now:");
    match yaw_raw {
        Some(r) => { d.set_text_color(AMBER, BG); let _ = d.println(&format!("{:4}", r)); }
        None    => { d.set_text_color(RED,   BG); let _ = d.println(" ---"); }
    }
...
fn main() -> ExampleResult {
    ...
    let mut servos = match StackChanServos::new() {
        Ok(mut s) => { s.set_torque(true); Some(s) }
        Err(_) => None,
    };
    ...
    loop {
        ...
        if m5.buttons.a().is_pressed() {
            ...
            if btn_a_frames > 0 && btn_a_frames < 40 {
                ...
                torque = false;
                if let Some(ref mut s) = servos { s.set_torque(false); }
                status = String::from("Torque OFF. Move servos by hand.");
            }
            btn_a_frames = 0;
        }
        if btn_a_frames >= 40 {
            if let Some(ref mut s) = servos { s.reset_calibration(); }
            torque = true;
            if let Some(ref mut s) = servos { s.set_torque(true); }
        ...
        draw_ui(&mut m5, yaw_zero, pitch_zero, yaw_raw, pitch_raw, torque, &status);
        m5.delay_ms(50);
    }
}

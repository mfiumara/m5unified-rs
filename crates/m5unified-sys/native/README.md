...
Cardputer firmware should also make the M5Cardputer Arduino library available to ESP-IDF/Arduino and define `M5UNIFIED_RS_USE_REAL_M5CARDPUTER=1`. Define `M5UNIFIED_RS_USE_ARDUINO_GPIO=1` when Arduino GPIO helpers are available and the firmware wants Grove GPIO helpers. Define `M5UNIFIED_RS_USE_ARDUINO_SD=1` when the Arduino `SPI`/`SD` libraries are available and the firmware wants the Cardputer microSD mount, status, and file helpers. Define `M5UNIFIED_RS_USE_ARDUINO_SERIAL=1` when Arduino `Serial1` is available and the firmware wants Grove UART helpers. Define `M5UNIFIED_RS_USE_ARDUINO_WIRE=1` when Arduino `Wire` is available and the firmware wants Grove I2C helpers. Define `M5UNIFIED_RS_USE_ARDUINO_IRREMOTE=1` when Arduino-IRremote is available and the firmware wants NEC IR transmit helpers. Set `M5UNIFIED_RS_ENABLE_STACKCHAN_BSP=ON` only in firmware that links StackChan-BSP and wants the official Stack-chan Motion hooks. The repository's `firmware/cardputer-keyboard` package shows the intended CMake wiring for that path.

The Stack-chan BSP hooks are an optional firmware boundary for official CoreS3 bodies: `begin` calls `M5StackChan.begin()`, configures Motion auto-sync/torque-release behavior, and homes the head; `update` forwards to `M5StackChan.update()`; move/home/nod/shake/status forward to StackChan-BSP Motion. Without `M5UNIFIED_RS_USE_STACKCHAN_BSP`, these functions compile as unavailable stubs.

The PWM servo shim uses ESP-IDF LEDC directly and does not require Arduino GPIO. It is intentionally small: attach a pin/channel/timer at a servo frequency, write a pulse width, and detach. The safe `m5unified` crate layers the Stack-chan MCP x/y/speed contract on top only as a generic fallback for custom non-official PWM pan/tilt builds. Official Stack-chan CoreS3 hardware needs the BSP Motion path because servo power and VM_EN/IO expander setup are board-specific.
